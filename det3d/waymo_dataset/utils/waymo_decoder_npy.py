...
FEATURE_SPEC = tfds.features.FeaturesDict({
    ...
    'lidars': {
        'points':
            tfds.features.Tensor(shape=(None, 6), dtype=tf.float32)
    },
...
def decode_frame(frame):
  ...
  lidars = extract_points(frame.lasers,
                          ...
                          frame.pose)
  output_labels, output_raw_object_id_list = extract_objects(frame.laser_labels)
  ...
  example_data = {
      ...
      'output_raw_object_id_list': output_raw_object_id_list
...
def extract_points(lasers, laser_calibrations, frame_pose):
  ...
  for laser, calibration in lasers_with_calibration:
    ...
    all_points.extend(points.astype(np.float32))
...
def extract_objects(laser_labels):
  """Extract objects."""
  objects = []
  # print("len_laser_labels: ", len(laser_labels))
  output_raw_object_id_list = []
  output_labels = np.zeros((len(laser_labels), 16), dtype=np.float32)
  for object_id, label in enumerate(laser_labels):
    ...
    output_labels[object_id,0] = np.array(object_id, dtype=np.float32)# id
    output_labels[object_id,1] = np.array(category_label, dtype=np.float32)# category_label
    output_labels[object_id,2:9] = np.array([box.center_x, box.center_y, box.center_z,
                         box.length, box.width, box.height, box.heading],
                        dtype=np.float32) # x, y, z, l, w, h, ry
    
    output_labels[object_id,9] = np.array(num_lidar_points_in_box, dtype=np.float32) # num_lidar_points_in_box
    output_labels[object_id,10] = np.array(label.detection_difficulty_level, dtype=np.float32) # detection_difficulty_level
    output_labels[object_id,11] = np.array(combined_difficulty_level, dtype=np.float32) # combined_difficulty_level
    output_labels[object_id,12:14] = np.array(speed, dtype=np.float32) # speed
    output_labels[object_id,14:16] = np.array(accel, dtype=np.float32) # accel
    output_raw_object_id_list.append(label.id) # raw object id
#   print("len(output_raw_object_id_list): ", len(output_raw_object_id_list))
#   print("output_labels.shape: ", output_labels.shape)
  assert len(output_raw_object_id_list) == output_labels.shape[0]
  return output_labels, output_raw_object_id_list
