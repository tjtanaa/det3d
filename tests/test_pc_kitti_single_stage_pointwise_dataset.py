...
if __name__ == "__main__":
    ...
    print("============= Downsampled Sample ================")
    
    down_sampled_aug_dataset = PCKittiSingleStagePointwiseDataset(root_dir=dataset_path, split='train', 
                npoints =16384,
                classes =['Car'], random_select =True)

    start  = time.time()
    for i in range(32):
        sample_info = down_sampled_aug_dataset.get_rpn_sample(i) 
        # sample_info['pts_input'] = pts_input
        # sample_info['pts_rect'] = aug_pts_rect
        # sample_info['pts_features'] = ret_pts_features
        # sample_info['rpn_cls_label'] = rpn_cls_label
        ...
        # sample_info['pts_rect'][:,1] *= -1 # mirror the y axis
        # coors = sample_info['pts_rect']
        # intensity = np.zeros(len(coors))
        # intensity[:4096] = 1

        # # intensity = sample_info['pts_features'][:,0]

        # bbox_params = np.stack([sample_info['gt_boxes3d'][:,5], sample_info['gt_boxes3d'][:,3], sample_info['gt_boxes3d'][:,4],
        #                         sample_info['gt_boxes3d'][:,0], -(sample_info['gt_boxes3d'][:,1] - sample_info['gt_boxes3d'][:,3] / 2) , 
        #                         sample_info['gt_boxes3d'][:,2],
        #                         sample_info['gt_boxes3d'][:,6]], axis=1)
        # Converter.compile("downsampled_aug_sample_{}".format(i), coors=coors, intensity=intensity,
        #                 bbox_params=bbox_params)
